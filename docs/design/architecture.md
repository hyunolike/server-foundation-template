...
| 함께 보기 | [설계 캔버스](https://claude.ai/code/artifact/c67eedeb-8e3a-4dd0-bbd2-b08a359ae663) |
...
컨트롤러는 도메인 DTO를 그대로 반환한다. 표준 응답으로 감싸는 일은 `ResponseBodyAdvice` **한 곳**에서만 일어난다.
...
### 3.3 문서는 생성하고, 테스트로 검증한다

springdoc이 코드에서 OpenAPI를 뽑는다. 하지만 원칙 3.1 때문에 springdoc은 컨트롤러 시그니처만 보고 `UserResponse`를 문서에 적는다 — 실제로 나가는 건 `ApiResponse<UserResponse>`다. **이 간극이 이 템플릿이 존재하는 이유다.** 커스터마이저가 문서 쪽에 봉투를 되돌려 넣고, REST Docs 계약 테스트가 "문서와 실제 응답이 같은가"를 CI에서 확인한다.
...
| `foundation-web` | 봉투 Advice, 전역 예외 핸들러, TraceId 필터, 페이징 리졸버 | 표준화가 걸리는 지점 |
| `foundation-docs` | springdoc 커스터마이저, `@ApiErrorCodes`, Swagger UI 프리셋 | core의 에러 코드를 문서로 번역 |
| `foundation-observability` | MDC traceId, 구조화 로깅, 민감정보 마스킹 | 마스킹 규칙의 단일 출처 |
| `foundation-test` | 봉투 단언 헬퍼, 에러 코드 계약 테스트, REST Docs 지원 | `java-test-fixtures` |
...
| # | 지점 | 하는 일 |
| --- | --- | --- |
| 01 | `TraceIdFilter` | 요청당 traceId 생성 또는 `X-Request-Id` 승계 → MDC + 응답 헤더 `X-Trace-Id` |
| 02 | `RequestLoggingFilter` | 본문 캐싱 래퍼, 마스킹 후 한 줄 구조화 로그 |
| 03 | `SecurityFilterChain` *(선택)* | 인증. 실패도 **같은 봉투**로 나가도록 EntryPoint 교체 |
| 04 | `HandlerInterceptor` | 레이트리밋 · 멱등키 · 감사 로그 훅. 템플릿은 비워 둔다 |
| 05 | `@Valid` + ArgumentResolver | 요청 DTO 바인딩·검증. 페이징은 공용 리졸버로 통일 |
| 06 | Controller → Service | **봉투를 모르는 유일한 구간** |
| 07 | `ResponseEnvelopeAdvice` | `ApiResponse.success()`로 래핑, `meta` 채움 |
| 08 | Jackson Converter | 네이밍 · 날짜 · null 정책을 한 `ObjectMapper` 설정으로 적용 |

### 예외 경로

05~06 어디서 갈라지든 07과 **같은 모양으로** 합류한다.
...
| `Exception` (fallback) | `COMMON_INTERNAL_ERROR` | **내부 메시지 비공개**, traceId만 안내. 로그는 ERROR로 전량 |

### 봉투 제외 경로

`/actuator/**`, `/v3/api-docs/**`, `/swagger-ui/**`, 그리고 `application/json`이 아닌 응답(파일 다운로드 등)은 07을 건너뛴다. 프로퍼티로 열어 두되 기본값은 코드에 박는다 — **이 목록이 길어지면 표준이 무너진 신호다.**
...
// 200 OK
{
  ...
  "data": {
    ...
    "createdAt": "2026-09-14T08:12:05Z"
  },
  "error": null,
  "meta": {
    "traceId": "01J9F2X7QK8M3ZB4",
...
// 404 Not Found — 상태 코드는 그대로 쓴다
{
  ...
  "error": {
    ...
    "details": [
      { "field": "userId", "reason": "존재하지 않는 식별자" }
    ]
  },
  "meta": { "…": "성공 응답과 동일" }
}
...
| `data` | `T \| null` | 항상 | 실패 시 `null`. 본문 없는 성공(204)도 `null` |
...
enum class UserErrorCode(
    ...
    USER_NOT_FOUND("USER_NOT_FOUND", HttpStatus.NOT_FOUND, "error.user.notFound"),
    USER_EMAIL_DUPLICATED("USER_EMAIL_DUPLICATED", HttpStatus.CONFLICT, "error.user.emailDuplicated"),
}
```

작명 규칙은 `<DOMAIN>_<REASON>` 대문자 스네이크. 도메인 접두사는 필수고, 여러 도메인이 공유하는 실패만 `COMMON_`을 쓴다.
...
`foundation-test`의 계약 테스트가 빌드마다 확인한다 — 코드 문자열 중복 없음, `code`와 enum 이름 일치, 상태 누락 없음, 메시지 키가 메시지 번들에 실재함.
...
**전부 200으로 내리지 않는다.** 상태 코드는 전송 계층의 사실이다. 게이트웨이·모니터링·브라우저 캐시가 먼저 읽는 값을 거짓으로 만들면 관측이 무너진다. `success`는 보조 신호다.
...
| 단계 | 내용 |
| --- | --- |
| **작성** | 컨트롤러에 `@Operation`과 `@ApiErrorCodes`만 단다. 응답 스키마는 적지 않는다 |
| **생성** | springdoc이 스펙을 뽑고, 커스터마이저가 봉투 스키마와 에러 응답을 주입한다 |
| **검증** | REST Docs 테스트가 실제 응답으로 스니펫을 만든다. 문서와 응답이 다르면 테스트가 깨진다 |
| **발행** | `openapi.json`을 커밋하고 정적 문서를 배포한다. 스펙 변경이 PR diff에 남는다 |
...
/api/v1/users/{userId}:
  get:
    ...
    responses:
      '200':
        schema: ApiResponse<UserResponse>   # EnvelopeSchemaCustomizer 가 주입
        headers: [ X-Trace-Id ]             # 주입
      '404':
        code: USER_NOT_FOUND                # @ApiErrorCodes 에서 주입
        description: 사용자를 찾을 수 없습니다.
        schema: ApiErrorResponse
      '500':
        code: COMMON_INTERNAL_ERROR         # 전역 기본 응답
        schema: ApiErrorResponse
```
...
| 전역 기본 응답 (400 · 401 · 500) | 결정의 배경 — ADR |

### 7.5 CI 게이트

| 태스크 | 막는 것 |
| --- | --- |
| `gradle test` | REST Docs 스니펫은 테스트가 통과해야 나온다. 없으면 문서 빌드 실패 |
| `openapi:verify` | 스펙을 재생성해 커밋된 `openapi.json`과 비교. 차이가 있으면 실패 — 갱신 누락 차단 |
| `openapi:diff` | 기준 브랜치 대비 필드 삭제 · 타입 변경 · 에러 코드 제거를 찾아 PR 코멘트 |
...
│  └─ response/ApiResponse.kt           ← 봉투 + meta
│     response/PageResponse.kt
│     error/ErrorCode.kt                ← 계약 인터페이스
│     error/CommonErrorCode.kt
│     error/BusinessException.kt
│
├─ foundation-web/
│  └─ advice/ResponseEnvelopeAdvice.kt  ← 봉투를 씌우는 유일한 지점
│     advice/GlobalExceptionHandler.kt
│     filter/TraceIdFilter.kt
│     filter/RequestLoggingFilter.kt
│     resolver/PageRequestResolver.kt
│     config/WebAutoConfiguration.kt    ← 의존성만 추가하면 켜진다
│
├─ foundation-docs/
│  └─ ApiErrorCodes.kt                  ← 엔드포인트에 다는 어노테이션
│     EnvelopeSchemaCustomizer.kt       ← 봉투를 문서에 되돌려 넣는다
│     ErrorResponseCustomizer.kt
│     DocsAutoConfiguration.kt
│
├─ foundation-observability/
│  └─ logging/StructuredLogEncoder.kt
│     logging/MaskingRules.kt           ← 마스킹 규칙의 단일 출처
│
├─ foundation-test/                     test-fixtures
│  └─ ApiResponseAssertions.kt
│     ErrorCodeContractTest.kt
│     RestDocsSupport.kt
│
├─ sample-api/                          여기부터가 복제해서 고칠 코드
...
- **설정은 자동으로.** 모든 foundation 모듈은 auto-configuration으로 등록된다. 애플리케이션에 복붙할 `@Configuration`이 없어야 표준이 유지된다.
- **`openapi.json`을 커밋한다.** 생성물이지만 리뷰 대상이다. diff에 스펙 변경이 보여야 호환성 깨짐을 PR에서 잡는다.
...
| 단계 | 이름 | 산출물 | 완료 기준 |
| --- | --- | --- | --- |
| **M0** | 골격 | 멀티모듈 6개, 버전 카탈로그, ktlint/detekt, CI 스켈레톤 | 빈 모듈 상태로 `gradlew build`가 CI에서 통과 |
| **M1** | 계약 | `foundation-core` — 봉투, 페이지, `ErrorCode`, 공통 코드 | Spring 의존 0, 에러 코드 계약 테스트 통과 |
| **M2** | 표준화 | `foundation-web` — 봉투 Advice, 예외 핸들러, TraceId, 페이징 | 성공·검증실패·미처리예외 세 응답이 모두 같은 봉투 |
| **M3** | 문서화 | `foundation-docs` — 스키마 커스터마이저, `@ApiErrorCodes` | Swagger UI에서 200이 봉투로, 404가 실제 코드로 보인다 |
| **M4** | 검증 | REST Docs 지원, `openapi:verify` / `openapi:diff` | 응답 필드를 하나 바꾸면 문서 갱신 없이는 CI가 빨개진다 |
| **M5** | 템플릿화 | 관측성 모듈, GitHub template, 패키지명 치환 스크립트, README | 새 저장소에서 첫 엔드포인트와 문서까지 10분 |
...
5. **버전 고정.** 본문의 Kotlin 2.x / Boot 3.x / JDK 21은 제안값이다. M0에서 정확한 버전으로 고정한다.
